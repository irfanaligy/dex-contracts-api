...
-- | Unsafe as it takes NFT's asset class where this NFT might not belong to the given version.
unsafeFetchPartialOrderConfig' :: forall v m a. (GYDexApiQueryMonad m a, SingPOCVersionI v) => GYAssetClass -> m (RefPocd v)
...
fetchPartialOrderConfig' :: forall v m a. (GYDexApiQueryMonad m a, SingPOCVersionI v) => PORefs -> m (RefPocd v)
