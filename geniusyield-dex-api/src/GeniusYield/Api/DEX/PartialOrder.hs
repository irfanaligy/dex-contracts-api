...
import GeniusYield.Api.DEX.Exceptions
...
import GeniusYield.Scripts.DEX.Version (defaultPOCVersion)
...
instance IsGYApiError PodException where
  ...
  toApiError (PodRequestedAmountGreaterOrEqualToOfferedAmount reqAmt offeredAmt) =
    GYApiError
      ...
      , gaeHttpStatus = status400
      , gaeMsg = Txt.pack $ printf "Requested Amount was: %s but offered amount is %s" (show reqAmt) (show offeredAmt)
      }
  ...
  toApiError (PodNonDifferentAssets offeredAsset) =
    GYApiError
      ...
      , gaeHttpStatus = status400
      , gaeMsg = Txt.pack $ "Offered asset is same as asked asset, which is: " ++ show offeredAsset
      }
  toApiError (PodEndEarlierThanStart startTime endTime) =
    GYApiError
      { gaeErrorCode = "END_EARLIER_THAN_START"
      , gaeHttpStatus = status400
      , gaeMsg = Txt.pack $ "End time is earlier than start. Start time: " ++ show startTime ++ ", end time: " ++ show endTime
      }
  toApiError (PodNegativeFrontendFee fee) =
    GYApiError
      { gaeErrorCode = "NEGATIVE_FRONTEND_FEE"
      , gaeHttpStatus = status400
      , gaeMsg = Txt.pack $ "Negative frontend fee: " ++ show fee
      }
  toApiError (PodNotAllOrderRefsPresent missingRefs) =
    GYApiError
      { gaeErrorCode = "MISSING_REFS"
      , gaeHttpStatus = status400
      , gaeMsg = Txt.pack $ "Not all of given references are present, missing ones: " ++ show missingRefs
      }
  toApiError (PodOrderDoesntBelongToScript ref) =
    GYApiError
      { gaeErrorCode = "ORDER_DOESNT_BELONG_TO_SCRIPT"
      , gaeHttpStatus = status400
      , gaeMsg = Txt.pack $ "Order doesn't belong to supported swap script credentials: " ++ show ref
      }
...
partialOrderInfoToIn
  ...
  -> PartialOrderAction
  -> GYTxIn PlutusV2
partialOrderInfoToIn GYCompiledScripts {dexPartialOrderValidator} pocVersion pors PartialOrderInfo {..} oa =
  let SomePORef (PORef {..}) = selectPor pors pocVersion
  in GYTxIn
       { gyTxInTxOutRef = poiRef
       , gyTxInWitness =
           GYTxInWitnessScript
             (GYInReference porValRef $ validatorToScript $ dexPartialOrderValidator pocVersion porRefNft)
             (Just poiRawDatum)
...
#ifdef USE_CEILING_FOR_V1_1
roundFunctionForPOCVersion1_1 = ceiling
#else
...
expectedPaymentWithDeposit poi@PartialOrderInfo {..} isCompleteFill =
  let
    toSubtract = valueSingleton (GYToken poiNFTCS poiNFT) 1 <> valueSingleton poiOfferedAsset (toInteger poiOfferedAmount) <> poiGetContainedFeeValue poi
    toAdd = if isCompleteFill then partialOrderPrice poi poiOfferedAmount else mempty
  in
    poiUTxOValue <> toAdd `valueMinus` toSubtract

-------------------------------------------------------------------------------
-- script address
-------------------------------------------------------------------------------

partialOrderAddr :: forall v m. (GYApiQueryMonad m, SingPOCVersionI v) => PORef v -> m GYAddress
partialOrderAddr PORef {..} = do
  gycs <- ask
  scriptAddress $ dexPartialOrderValidator gycs (fromSingPOCVersion $ singPOCVersion @v) porRefNft

partialOrderAddrTuple :: GYApiQueryMonad m => PORefs -> m (GYAddress :!: GYAddress)
...
partialOrderNftPolicy'
  ...
  -> GYScript PlutusV2
  -- ^ The minting policy of the partial order NFT.
partialOrderNftPolicy' gycs PORef {..} =
  dexPartialOrderNftPolicy gycs (fromSingPOCVersion $ singPOCVersion @v) porRefNft
...
getPartialOrderVersion' :: GYApiQueryMonad m => (Maybe GYPaymentCredential :!: Maybe GYPaymentCredential) -> (GYAddress :!: GYTxOutRef) -> m POCVersion
...
getPartialOrdersInfos' :: GYApiQueryMonad m => PORefs -> [(GYTxOutRef, Natural)] -> m [(PartialOrderInfo, Natural)]
...
placePartialOrder'
  ...
  -- ^ Stake credential of user. We do not support pointer reference.
  -> GYTxOutRef
  -> PartialOrderConfigDatumF GYAddress
  -> m (GYTxSkeleton PlutusV2)
placePartialOrder' pors = placePartialOrderWithVersion' pors defaultPOCVersion
...
placePartialOrderWithVersion' pors pocVersion addr (offerAmt, offerAC) priceAC price start end addLov addOff stakeCred cfgRef pocd = do
  ...
  let
    ...
    makerFeeFlat = fromIntegral addLov + pocdMakerFeeFlat pocd
    makerFeeOff = (+) (fromIntegral addOff) $ roundFunctionForPOCVersion pocVersion $ toRational offerAmt * rationalToGHC (pocdMakerFeeRatio pocd)
    makerFee =
    ...
    offerV =
      valueSingleton offerAC offerAmt'
        <> nftV
        <> valueFromLovelace (toInteger $ pocdMinDeposit pocd)
        <> makerFee
    containedFee =
      PartialOrderContainedFee
    ...
    od =
      PartialOrderDatum
        ...
        , podTakerLovelaceFlatFee = pocdTakerFee pocd
...
mkSkeletonCompletelyFillPartialOrder pors oi@PartialOrderInfo {..} mRefPocd addTakerFee = do
  ...
  let
    containedFee = poiGetContainedFeeValue oi
...
addCfgRefInputs versionsSet cfgRefs =
  let
    RefPocd (cfgRefV1 :!: _) = selectV1RefPocd cfgRefs
    RefPocd (cfgRefV1_1 :!: _) = selectV1_1RefPocd cfgRefs
  in
    ( if Set.member POCVersion1 versionsSet
...
preferentiallySelectLatestPocd :: Set POCVersion -> RefPocds -> PartialOrderConfigDatum
preferentiallySelectLatestPocd versionsSet cfgRefs =
  let
    overallVersion = preferentiallySelectLatestVersion versionsSet
...
cancelMultiplePartialOrders' pors ois = do
  ...
  let
    (!feeOutputMap, !totalRequiredFees, !accumulatedSkeleton) =
      foldl'
        ( \(!mapAcc, !feeAcc, !skelAcc) poi@PartialOrderInfo {..} ->
            let
              por@(SomePORef PORef {..}) = selectPor pors poiVersion
              skelAdd =
                ...
                then (mapAcc, feeAcc, skelAcc <> skelAdd)
                  ...
                  let
                    reqContainedFee =
                      ...
                      in
                        POIContainedFee {poifLovelaces = poifLovelaces, poifOfferedTokens = poifOfferedTokens - feeToRefund, poifAskedTokens = poifAskedTokens}
                    reqContainedFeeValue = poiContainedFeeToValue reqContainedFee poiOfferedAsset poiAskedAsset
                  in
                    (PlutusTx.unionWith (<>) mapAcc (PlutusTx.singleton (txOutRefToPlutus poiRef) (valueToPlutus reqContainedFeeValue)), feeAcc <> reqContainedFeeValue, skelAcc <> skelAdd)
        )
    ...
    feeOutput
      ...
      | otherwise =
          mustHaveOutput $ mkGYTxOut (pocdFeeAddr pocd) totalRequiredFees $ datumFromPlutusData $ PartialOrderFeeOutput feeOutputMap mempty Nothing
  pure
    $ feeOutput
      <> accumulatedSkeleton
      <> addCfgRefInputs versionsSet cfgRefs
...
__NOTE:__ This adds additional taker fee only if fee output is being generated, i.e., only if at least one order is being completely filled.
-}
fillMultiplePartialOrders'
  :: (GYApiMonad m, HasCallStack)
  => PORefs
...
fillMultiplePartialOrders' pors orders mRefPocd addTakerFee = do
  ...
  gycs <- ask
  let
    versionsSet = getVersionsInOrders $ map fst orders
    ...
    buildWithFeeOutput = do
      let
        (!feeOutputMap, !totalContainedFee, !maxTakerFee) =
          foldl'
        ...
        feeOutput
          | fee == mempty = mempty
          | otherwise =
              mustHaveOutput $ mkGYTxOut (pocdFeeAddr pocd) (fee <> addTakerFee) $ datumFromPlutusData $ PartialOrderFeeOutput feeOutputMap mempty Nothing
      foldlM
              ...
              skel =
                if amt == poiOfferedAmount
                  then
                    ...
                    let
                      price' = partialOrderPrice poi amt
                      od =
                        partialOrderInfoToPartialOrderDatum
                          poi
                            { poiOfferedAmount = poiOfferedAmount - amt
