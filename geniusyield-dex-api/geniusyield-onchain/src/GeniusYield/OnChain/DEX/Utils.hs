...
import GeniusYield.OnChain.DEX.PartialOrder.Types
  ( PMaybePPOSIXTimeData (PPDJust, PPDNothing)
...
pfindMarkedRefInput = phoistAcyclic
  ...
  where
    ...
    matches = phoistAcyclic
      $ plam
      $ \reqAddr reqAsset txininfo ->
        plet (pfield @"resolved" # txininfo)
          $ \resolved ->
            pletFields @'["address", "value"] resolved $ \resolved' ->
              getField @"address" resolved'
                #== reqAddr
                #&& passetClassValueOf
                # getField @"value" resolved'
                # reqAsset
                #== 1
...
pfindFeeOutput = phoistAcyclic
  $ plam
  $ \outputs datums feeAddr takerFeeInt x f ->
    unTermCont
...
ppartialOrderToken = phoistAcyclic
  $ plam
  $ \ownAddr nftSymbol mint out -> unTermCont $ do
  ...
  where
    ...
    f nftSymbol mint kv _ =
      plet (pfromData $ pfstBuiltin # kv) $ \nftTkName ->
        pif
          (pfromData (psndBuiltin # kv) #== 1) -- We require non-zero value, to safeguard if map is not normalized.
...
pcontinuingPartialOrder = phoistAcyclic
  $ plam
  $ \ownAddr nftSymbol nftTkName mint outputs ->
  ...
  where
    ...
    go ownAddr nftSymbol nftTkName mint =
      precList
        ( \self out xs -> plam $ \b ->
