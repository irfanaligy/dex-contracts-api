    ...
    pmatchC oa >>= \case
      PPartialCancel _ -> do
        ...
        pmatchC (partialFills #== 0 #|| curContainedFee #== mempty) >>= \case
          -- Note that later we subtract from this @curContainedFee@ to know for the required contained fee, but if required contained fee is @mempty@ then it can be mathematically shown that @curContainedFee@ is also @mempty@.
      ...
      PPartialFill pf -> do
        ...
        pguardC
          ...
          (0 #< amt)
        pguardC
          "amount must be less than offered amount"
          (amt #< offeredAmount)
        ...
        hasFeeOutput <- pletC
          $ pfindFeeOutput
            ...
            #$ plam
          $ \_ -> pcon PTrue -- fee output containing taker fees found
        ...
        pmatchC (pcontinuingPartialOrder # getField @"address" ownInpUtxo # nftSymbol # nftTkName # mint # outputs) >>= \case
          ...
          PJust p -> do
            ...
            pguardC
              "wrong datum in continuing output"
              (actualDatum #== toDatum expectedDatum)
            pguardC
              "continuing output's value should have <= 10 tokens" -- To prevent against token dust attack as order author should always be able to cancel their order. Note that filler cannot increase datum size arbitrarily.
