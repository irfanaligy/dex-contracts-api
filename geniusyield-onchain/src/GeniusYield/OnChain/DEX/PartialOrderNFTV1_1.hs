...
import Plutarch.Rational qualified as Rational
    ...
    checkOutput = plam
      $ \sh deposit outputs datums cs tn makerFeeFlatContract makerFeeRatioContract takerFeeFlatContract -> unTermCont $ do
        ...
        pguardC "fees too low"
          $ makerFeeFlatContract
            #<= pfromData containedFeeLov
            #&& pfloor
            # ((prationalFromData # makerFeeRatioContract) #* (Rational.pfromInteger # offeredAmount))
            #<= pfromData containedFeeOff
